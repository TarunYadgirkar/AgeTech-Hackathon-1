...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!RETELL_API_KEY || !RETELL_AGENT_ID || !RETELL_FROM_NUMBER) {
    res.status(503).json({ error: 'check-in not configured' });
    return;
  }

  const body = req.body as Record<string, unknown>;
  const to = typeof body?.to === 'string' ? body.to.trim() : '';
  const target = typeof body?.target === 'string' ? body.target.trim() : 'Resident';
  ...
  try {
    ...
    console.error('[check-in] Retell call failed:', e instanceof Error ? e.message : e);
    res.status(500).json({ error: 'check-in call failed' });
  }
}
