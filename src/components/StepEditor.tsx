...
function StepRow({ step, index, total, onRemove, onMoveUp, onMoveDown, onUpdate }: RowProps) {
  ...
  return (
    <div className={`bg-slate-800 border rounded-xl p-3 space-y-2.5 ${is911 ? 'border-red-800/60' : 'border-slate-700'}`}>
      ...
      {/* Phone number row — shown for callable steps only */}
      {!is911 && (
        <div className="flex items-center gap-2 pl-8">
          ...
          <input
            ...
            placeholder="+1 (415) 555-1234  — leave blank to skip call"
            className="flex-1 text-xs bg-slate-700/60 border border-slate-600/60 text-slate-300 placeholder-slate-600 rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-blue-500/50 min-w-0"
          />
        </div>
      )}
    </div>
